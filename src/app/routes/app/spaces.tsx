...
import { BookOpen, Users, LayoutGrid, Plus, Settings, MessageCircle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useState } from "react";
import ChatModal from "@/features/chat/components/ChatModal"; // 👈 РАСКОММЕНТИРОВАНО

const spaceRoute = () => {
    // Состояние для открытия/закрытия чата
    ...
    return (
        <>
            ...
            {/* Кнопка открытия чата - РАСКОММЕНТИРОВАНО */}
            <button
                onClick={() => setIsChatOpen(true)}
                className="fixed bottom-6 right-6 bg-[#007BFF] text-white p-4 rounded-full shadow-lg hover:bg-blue-600 transition-all z-40 hover:scale-110 active:scale-95"
                title="Открыть чат"
            >
                <MessageCircle className="w-6 h-6" />
            </button>

            {/* Модальное окно чата - РАСКОММЕНТИРОВАНО */}
